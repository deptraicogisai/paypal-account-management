...
const SendPartialRefundModal = (props: SendRefundProps) => {
    ...
    const offerPartialRefund = async () => {
        try {
            var errors = validateFields();
            setFieldErrors(errors);
            if (Object.keys(errors).length === 0) {
                var result = await dispute.offerPartialRefund(props.dispute_id, formData);
                if (result.success) {
                    props.onHide();
                    props.onHideAndShowNotification("The partial refund offer has been sent");
                } else {
    ...
    return (
        <Modal show={props.show} centered onHide={props.onHide}>
            ...
            <Modal.Body>
                ...
                <Button className="reponse-disute-button" variant="dark" onClick={offerPartialRefund}>Send Offer</Button>
            </Modal.Body>
        </Modal>
    )
}

export default SendPartialRefundModal;